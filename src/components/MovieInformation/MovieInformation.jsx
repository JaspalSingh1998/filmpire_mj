...
const MovieInformation = () => {
  ...
  return (
      ...
      <Grid item container direction='column' lg={7}>
        ...
        <Grid item className={classes.genresContainer}>
          {data?.genres?.map((genre, index) => (
            <Link key={genre.name} className={classes.links} to='/' onClick={() => dispatch(selectGenreOrCategory(genre.id))}>
              <img src={genreIcons[genre.name.toLowerCase()]} alt="GenreImage" className={classes.genreImage} height={30}/>
        ...
        <Grid item container style={{marginTop: '2rem'}}>
          <div className={classes.buttonsContainer}>
            ...
            <Grid item xs={12} sm={6} className={classes.buttonsContainer}>
              <ButtonGroup size='medium' variant='outlined'>
                <Button onClick={addToFavourites} endIcon={isMovieFavourite ? <FavoriteBorderOutlined /> : <Favorite /> }>{isMovieFavourite ? 'Unfavorite' : 'Favorite'}</Button>
    ...
    {/* Loop through the recommend movies */}
    {recommendations ? 
    (
  ...
  <Modal closeAfterTransition className={classes.modal} open={open} onClose={() => setOpen(false)}>
      {
        data?.videos?.results?.length > 0 && (
          <iframe allow='autoPlay' src={`https://www.youtube.com/embed/${data.videos.results[0].key}`} frameBorder="0" title='Trailer' autoPlay className={classes.videos}></iframe>
        )
      }
  </Modal>
  </Grid>
